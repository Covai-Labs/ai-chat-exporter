...
export class ClaudeParser extends ChatParser {
    ...
    async parse() {
        ...
        const selectors = [
            ...
            '.font-claude-message',
            '.grid.gap-2', // Sometimes used for assistant rows
            '.artifact-block-cell'
        ].join(', ');
        ...
        const validFallbacks = fallbackCandidates.filter(fallback => {
            const overlapsWithError = strictCandidates.some(strict =>
                strict.contains(fallback) || fallback.contains(strict)
            );
        ...
        for (const el of allElements) {
            ...
            } else if (el.matches('.font-claude-message') || el.matches('.font-claude-response') || el.matches('div.font-serif')) {
                role = 'Claude';
                // Convert HTML to markdown  
                const clone = el.cloneNode(true);
                clone.querySelectorAll('button').forEach(btn => btn.remove());
                ...
                if (index !== undefined) {
                    ...
                    if (info) {
                        ...
                        if (artLang === 'markdown' || artLang === 'text') {
                            ...
                            const quotedContent = artContent.split('\n').map(line => `> ${line}`).join('\n');
                            content = `\n\n> **Artifact: ${artTitle}**\n\n${quotedContent}\n\n`;
                        } else {
                            content = `\n\n> **Artifact: ${artTitle}**\n\`\`\`${artLang}\n${artContent}\n\`\`\`\n\n`;
                        }
                        ...
                        const header = el.querySelector('.flex.items-center.gap-2') || el.querySelector('.font-bold');
