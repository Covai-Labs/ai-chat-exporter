...
export class MetaParser extends ChatParser {
    ...
    async parse() {
        ...
        const title = (titleInput && titleInput.value) ? titleInput.value : (titleButton ? titleButton.innerText : 'Meta AI Session');
        ...
        messageElements.forEach(el => {
            ...
            const isUser = el.matches('[data-message-type="user"]') || (el.getAttribute('data-message-id') && el.getAttribute('data-message-id').endsWith('_user'));
            const isAssistant = el.matches('[data-testid="assistant-message"]') || (el.getAttribute('data-message-id') && el.getAttribute('data-message-id').endsWith('_assistant'));

            if (isUser) {
                role = 'User';
                // User text is usually in a span with text-response class or simply pre-wrap
                const textEl = el.querySelector('[data-slot="text"].text-response') || el.querySelector('.whitespace-pre-wrap');
                if (textEl) {
                ...
                const contentEl = el.querySelector('.markdown-content') || el.querySelector('.ur-markdown') || el.querySelector('.prose');
                if (contentEl) {
