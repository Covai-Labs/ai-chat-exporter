...
export class GeminiParser extends ChatParser {
    ...
    async parse() {
        ...
        try {
            ...
            const deepResearchTitle = document.querySelector('h1, .title, .conversation-title, [data-testid="title"]');
            if (deepResearchTitle) {
                const text = deepResearchTitle.innerText.trim();
                console.log('[Gemini Parser] Found potential title:', text);
                if (text.length > 5 && !text.includes('Gemini') && !text.includes('Help') && !text.includes('Settings')) {
                    title = text;
            ...
            if (!title) {
                ...
                const contentTitles = document.querySelectorAll('main h1, main h2, article h1, article h2, .content h1, .content h2');
                    ...
                    if (userQuery) {
                        ...
                        if (queryText) {
                            ...
                            if (userText && !seenTexts.has(userText)) {
                                ...
                                console.log('[Gemini Parser] Added user message:', userText.substring(0, 50) + '...');
                            }
                        }
                    }
                        ...
                        if (messageContent) {
                            ...
                            if (markdownDiv) {
                                ...
                                if (text && text.trim() && !seenTexts.has(text.trim())) {
                                    ...
                                    console.log('[Gemini Parser] Added model message:', text.substring(0, 50) + '...');
                                }
                            } else {
                                console.log('[Gemini Parser] No markdown div found, trying comprehensive content extraction...');
                                ...
                                for (const selector of nestedSelectors) {
                                    ...
                                    console.log('[Gemini Parser] Looking for nested elements with selector:', selector, 'found:', nestedElements.length);
                                    
                                    nestedElements.forEach(element => {
                                        const text = element.innerText.trim();
                                        if (text.length > 100) {
                                            console.log('[Gemini Parser] Found nested content, length:', text.length);
                                            console.log('[Gemini Parser] Nested content preview:', text.substring(0, 200) + '...');
                                            
                                            const isDeepResearch = text.includes('research') || 
                                            ...
                                            if (!seenTexts.has(text)) {
                                                ...
                                                console.log('[Gemini Parser] Added nested content, Deep Research:', isDeepResearch);
                                                foundContent = true;
                                            }
                                        }
                                    });
                                    ...
                                    if (parentContainer) {
                                        ...
                                        Array.from(siblings).forEach(sibling => {
                                            if (sibling !== modelResponse) {
                                                ...
                                                if (text.length > 200) {
                                                    console.log('[Gemini Parser] Found sibling content, length:', text.length);
                                                    console.log('[Gemini Parser] Sibling content preview:', text.substring(0, 200) + '...');
                                ...
                                if (!foundContent) {
                                    ...
                                    if (containerText.length > 500) {
                                        console.log('[Gemini Parser] Full container content preview:', containerText.substring(0, 200) + '...');
                                ...
                                if (!foundContent) {
                                    ...
                                    console.log('[Gemini Parser] Last resort text preview:', allText.substring(0, 200) + '...');
                                    
                                    if (allText && allText.length > 50) {
                                        // Check if this looks like Deep Research content
                                        const isDeepResearch = allText.includes('research') || 
                                        ...
                                        if (!seenTexts.has(allText)) {
                                            ...
                                            console.log('[Gemini Parser] Added last resort model message, Deep Research:', isDeepResearch);
                                        }
                                    }
                                }
                            }
                        } else {
                            console.log('[Gemini Parser] No message-content found, trying direct model-response text...');
                            ...
                            if (directText && directText.length > 50) {
                                const isDeepResearch = directText.includes('research') || 
                                ...
                                if (!seenTexts.has(directText)) {
                                    ...
                                    console.log('[Gemini Parser] Added direct model message, Deep Research:', isDeepResearch);
                                }
                            }
                        }
                    }
            ...
            if (!extractionAttempted) {
                console.log('[Gemini Parser] Strategy 2: No conversation containers found, trying Deep Research panel...');
                extractionAttempted = true;
                    ...
                    if (panelContent.length > 0) {
                        panelContent.forEach(section => {
                            if (section.content && !seenTexts.has(section.content)) {
                                ...
                                console.log('[Gemini Parser] Added panel section, role:', section.role, 'length:', section.content.length);
                            }
                        });
                    } else {
    ...
    extractDeepResearchSections(contentElement) {
        ...
        const patterns = [
            ...
            {
                promptRegex: /(?:Prompt|You said)[:\s]*\n*([\s\S]*?)(?=\n\s*(?:Response|I've completed|Generating|Start research)|$)/i,
                responseRegex: /(?:Response|I've completed|Generating|Start research)[:\s]*\n*([\s\S]*?)(?=\n\s*(?:Prompt|You said)|$)/i
            },
            ...
            {
                promptRegex: /(?:Research plan|Research query|What is|How has|What's the projection)[:\s]*\n*([\s\S]*?)(?=\n\s*(?:I've completed|Research|Analysis|Results)|$)/i,
                responseRegex: /(?:I've completed|Research|Analysis|Results|Findings)[:\s]*\n*([\s\S]*?)(?=\n\s*(?:Research plan|Research query|What is|How has)|$)/i
            }
        ];
        ...
        const userElements = contentElement.querySelectorAll('.user-query, .prompt, .question, [data-role="user"]');
        ...
        const responseElements = contentElement.querySelectorAll('.model-response, .response, .answer, [data-role="model"], .research-content');
    ...
    extractDeepResearchPanelContent(panelElement) {
        ...
        console.log('[Gemini Parser] Panel innerText length:', panelElement.innerText ? panelElement.innerText.length : 0);
